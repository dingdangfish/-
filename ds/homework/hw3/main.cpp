...
HuffmanNode* buildHuffmanTree(map<char, int> frequencies) {
    ...
    while (pq.size() > 1) {
        ...
        HuffmanNode* new_node = new HuffmanNode('#', left->freq + right->freq, left, right);
        pq.push(new_node);
    }
    return pq.top();
}
...
// Huffman编码函数
string huffmanEncode(string text, map<char, string>& codes) {
    string encoded = "";
    for (char ch : text) {
        encoded += codes[ch];
    }
...
// 统计字符出现频率
map<char, int> countFrequencies(string text) {
...
// 统计26个英文字母出现次数
vector<int> countAlphabets(string text) {
...
int main() {
    ...
    // 统计26个英文字母出现次数
    vector<int> count = countAlphabets(text);
    cout << "英文字母出现次数:" << endl;
